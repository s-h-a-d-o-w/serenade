...
export default class Stream {
  ...
  constructor(
    ...
    setInterval(() => {
      ...
      this.log.verbose("Sending keepalive");
  ...
  async connect(chunkManager: ChunkManager, custom: Custom, executor: Executor): Promise<boolean> {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      this.coreSocket.on("open", () => {
        this.log.verbose("Stream connected");
  ...
  disconnect() {
    ...
    this.log.verbose("Stream disconnected");
  ...
  sendCallbackRequest(callbackRequest: core.ICallbackRequest) {
    this.log.verbose(`Sending callback request: ${callbackRequest.type}`);
    this.send(this.coreSocket, {
      callbackRequest,
    });
  }
  ...
  async sendEditorStateRequest(clipboard: boolean = false, editorState?: any): Promise<any> {
    this.log.verbose("Sending editor state");
    if (!editorState) {
  ...
  async sendInitializeRequest(): Promise<any> {
    this.log.verbose("Sending initialize request");
    this.send(this.coreSocket, {
      initializeRequest: {
  ...
  async sendTextRequest(text: string, includeAlternatives: boolean) {
    this.log.verbose(`Sending text request: ${text}, ${includeAlternatives}`);
