...
export default class ChunkManager {
  ...
  private async flush() {
    ...
    while (this.buffer.length > 0) {
      ...
      if (request.requestType != "audio") {
        this.log.verbose(`Flushing ${request.requestType}`);
      }
  ...
  async attemptToEvaluateChunk(chunk: Chunk): Promise<any> {
    if (this.chunkQueue.size() == 0) {
      this.log.verbose(`Attempt to evaluate chunk, but empty chunk queue`);
      return;
    }
    ...
    if (!chunk.reverted && chunk.executed) {
      this.log.verbose(`Not executing chunk ${chunk.id}: already executed`);
      return;
    }

    if (chunk.id != current.id) {
      this.log.verbose(`Not executing chunk ${chunk.id}: new chunk started`);
      return;
    }

    if (!chunk.reverted && !chunk.response) {
      this.log.verbose(`Not executing chunk ${chunk.id}: no final response yet`);
      return;
    }

    if (chunk.reverted && !chunk.revertedResponse) {
      this.log.verbose(`Not executing chunk ${chunk.id}: no reverted response yet`);
      return;
    }

    if (!this.reachedSilenceThreshold(chunk)) {
      this.log.verbose(`Not executing chunk ${chunk.id}: waiting for silence`);
      return;
    }
    ...
    if (
      ...
      this.log.verbose(`Not executing chunk ${chunk.id}: no alternatives or execute`);
    ...
    if (chunk.response && chunk.response.final && this.shouldAppendToPrevious(chunk.response)) {
      this.log.verbose(`Appending to previous ${chunk.id}`);
    ...
    this.log.verbose(`Setting partial to false`);
    ...
    this.log.verbose(`Executing chunk ${chunk.id}`);
  ...
  async onCommandsResponse(response: core.ICommandsResponse) {
    ...
    if (!chunk) {
      this.log.verbose(`No chunk found for ${response.chunkId!}`);
      return;
    }
    ...
    if (!this.shouldAppendToPrevious(response)) {
      ...
      if (!isMetaResponse(response) && response.alternatives && response.alternatives.length > 0) {
        this.log.verbose(`Setting partial = ${partial}`);
        this.bridge.setState(
          {
            partial,
          },
  ...
  onAudio(audio: any, silence: number) {
    ...
    if (
      ...
      this.log.verbose(`Silence hit for ${current.id}`);
  ...
  async onChunkEnd() {
    ...
    this.log.verbose(`Chunk end for ${current.id}`);
  ...
  async onChunkStart(audio: any) {
    ...
    this.log.verbose(`Chunk start for ${id}`);
  ...
  startBuffering() {
    this.log.verbose("Buffering started");
    this.buffering = true;
  }

  async stopBufferingAndFlush() {
    this.log.verbose("Buffering stopped");
  ...
  async toggle(listening?: boolean) {
    ...
    this.log.verbose(`Toggling listening to ${listening}`);
    setTimeout(async () => {
