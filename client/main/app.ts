...
const CUSTOM_COMMANDS_SERVER_TIMEOUT = 5000;
...
export default class App {
  ...
  static async create() {
    ...
    log.verbose("create(): Creating main window...");
    ...
    log.verbose("create(): Creating mini mode window...");
    ...
    log.verbose("create(): Creating revision box window...");
    ...
    log.verbose("create(): Registering native theme updated event handler...");
    ...
    log.verbose("create(): Creating plugin manager...");
    const pluginManager = new PluginManager(settings);

    log.verbose("create(): Creating microphone...");
    ...
    log.verbose("create(): Creating active...");
    ...
    log.verbose("create(): Creating stream...");
    ...
    log.verbose("create(): Creating IPC server...");
    ...
    Promise.race([custom.start(), new Promise((resolve) => setTimeout(()=> resolve('timeout'), CUSTOM_COMMANDS_SERVER_TIMEOUT))]).then((result) => {
      if (result === 'timeout') {
        log.error("create(): Custom commands server can't be reached!");
      } else {
        log.verbose("create(): Custom commands server started");
      }
    });
    ...
    log.verbose("create(): Creating command handler...");
    ...
    log.verbose("create(): Creating renderer process event handlers...");
    ...
    log.verbose("create(): Sending all settings...");
    ...
    log.verbose("create(): Clearing alternatives and showing examples...");
    instance.clearAlternativesAndShowExamples();
    log.verbose("create(): Showing nux if needed...");
    nux.showIfNeeded();

    log.verbose("create(): Creating language switcher window...");
    ...
    log.verbose("create(): Creating settings window...");
