...
// Possible logs are in the serenade.log file in the .serenade directory

let commands: Record<string, any> = {};
let hints: Record<string, any> = {};
let words: Record<string, any> = {};
let onContextChangedCallback = (payload?: any) => {};
...
const createBuilder = (applications, languages, extensions, urls) => {
  ...
  return {
    ...
    snippet: (templated, generated, options, snippetType, disabled) => {
      const id = Math.random().toString();
      if (options && snippetType === undefined && disabled === undefined) {
...
const reload = async () => {
  commands = {};
  try {
    const files = await fg([`${scripts.replace(/\\/g, '/')}/**/*.js`], {dot: true, followSymbolicLinks: true, ignore: ["**/node_modules/**", "**/.git/**"]});
...
const main = () => {
  ...
  chokidar.watch(scripts, { ignoreInitial: true }).on("all", debounce(reload, 500));
};
